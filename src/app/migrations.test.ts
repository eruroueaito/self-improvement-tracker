...
import { createEmptyPersistedSnapshotV2 } from '../test/fixtures/persistedSnapshotV2';
import { migrateSnapshot, UnsupportedSchemaVersionError } from './migrations';

describe('migrateSnapshot', () => {
  it('migrates an independent v1 fixture through v2 to v3 without mutating facts', () => {
  ...
  it('migrates v2 settings while keeping AI and history disabled by default', () => {
    const legacy = createEmptyPersistedSnapshotV2();
    legacy.settings.ai.enabled = true;
    legacy.settings.ai.historyEnabled = true;
    const before = structuredClone(legacy);

    expect(migrateSnapshot(legacy)).toEqual({
      migratedFrom: 2,
      snapshot: {
        ...before,
        schemaVersion: 3,
  ...
  it('validates and clones current v3 snapshots without reporting a migration', () => {
    const current = {
  ...
  it.each([0, 4, 99])('rejects unsupported schema version %s', (schemaVersion) => {
  ...
  it('rejects unknown snapshot and nested settings fields', () => {
    ...
    expect(() => migrateSnapshot({
      ...createEmptyPersistedSnapshotV1(),
      schemaVersion: 2,
      settings: { ...createDefaultAppSettingsV2(), ai: { enabled: false, historyEnabled: false, apiKey: 'secret-value' } },
    })).toThrow(/不受支持的字段/);
    expect(() => migrateSnapshot({
      ...createEmptyPersistedSnapshotV1(),
      schemaVersion: 3,
