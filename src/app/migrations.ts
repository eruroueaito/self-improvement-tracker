 ...
 * 输入/输出：接收未知持久化值，返回 schemaVersion 3 快照与原始迁移版本
 * 依赖关系：版本化快照、事实校验、版本化 AppSettings、AI history 与快照预算
...
import { createDefaultAiProviderSettings } from '../modules/ai/providerConfig';
import {
...
import { assertSnapshotWithinBudget } from './snapshotBudget';
import {
...
const V3_KEYS = [...V2_KEYS, 'aiInteractions'] as const;
...
const validateV2 = (value: unknown): PersistedSnapshotV2 => {
  const snapshot = asRecord(value);
...
const validateV3 = (value: unknown): CurrentAppSnapshot => {
  const snapshot = asRecord(value);
  assertExactSnapshotKeys(snapshot, V3_KEYS);
  ...
  const current: CurrentAppSnapshot = {
    ...
    aiInteractions: normalizeAiInteractionHistory(snapshot.aiInteractions),
  };
  assertSnapshotWithinBudget(current);
  return current;
};

const v1ToV2 = (snapshot: PersistedSnapshotV1): PersistedSnapshotV2 => ({
  schemaVersion: 2,
...
const v2ToV3 = (snapshot: PersistedSnapshotV2): CurrentAppSnapshot => ({
  schemaVersion: CURRENT_SCHEMA_VERSION,
  goals: structuredClone(snapshot.goals),
  activities: structuredClone(snapshot.activities),
  recommendationRuns: structuredClone(snapshot.recommendationRuns),
  ...
  settings: {
    ...
    ai: {
      enabled: snapshot.settings.ai.enabled,
      goalDraftEnabled: false,
      historyEnabled: snapshot.settings.ai.historyEnabled,
...
export const migrateSnapshot = (value: unknown): MigrationResult => {
  const snapshot = asRecord(value);
  if (snapshot.schemaVersion === CURRENT_SCHEMA_VERSION) {
    return { snapshot: validateV3(snapshot), migratedFrom: null };
  }
  if (snapshot.schemaVersion === 2) {
    return { snapshot: validateV3(v2ToV3(validateV2(snapshot))), migratedFrom: 2 };
  }
  if (snapshot.schemaVersion === 1) {
    const migrated = v2ToV3(v1ToV2(validateV1(snapshot)));
    return { snapshot: validateV3(migrated), migratedFrom: 1 };
  }
  throw new UnsupportedSchemaVersionError(snapshot.schemaVersion);
};
