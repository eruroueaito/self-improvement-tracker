...
import { appendAiInteraction } from '../modules/ai/interactionLog';
import type {
  AiConnectionTestOutcome,
  AiGoalDraftOutcome,
  AiGoalDraftServicePort,
  TestAiConnectionInput,
} from './aiGoalDraftService';
import {
  buildExportEnvelope,
  buildRecoveryExport,
  summarizeImport,
...
export interface CreateGoalWithActivitiesResult {
  goal: Goal;
  activities: ActivityTemplate[];
}

export type AiHistoryWarning = 'history-limit' | 'storage' | null;
export type GenerateAiGoalDraftResult = AiGoalDraftOutcome & { historyWarning: AiHistoryWarning };
...
export class MvpApplication {
  ...
  private historyEnabledIntent = false;
  ...
  constructor(
    ...
    private readonly aiGoalDraftService?: AiGoalDraftServicePort,
  ) {}
  ...
  async initialize(): Promise<AppSnapshot> {
    ...
    this.historyEnabledIntent = this.current().settings.ai.historyEnabled;
    this.recoveryExport = null;
  ...
  async createGoal(goalInput: GoalInput, activityInput: ActivityInput): Promise<CreateGoalResult> {
    const created = await this.createGoalWithActivities(goalInput, [activityInput]);
    return { goal: created.goal, activity: created.activities[0]! };
  }

  async createGoalWithActivities(
    goalInput: GoalInput,
    activityInputs: readonly ActivityInput[],
  ): Promise<CreateGoalWithActivitiesResult> {
    const normalizedGoal = normalizeGoalInput(goalInput);
    if (activityInputs.length < 1 || activityInputs.length > 5) {
      throw new ValidationError('一个目标必须包含 1–5 个活动');
    }
    const normalizedActivities = activityInputs.map(normalizeActivityInput);
    return this.mutations.run(async () => {
      ...
      const activities = normalizedActivities.map((activity) =>
        this.buildActivity(goal.id, activity, now, this.ids.next()));
      const next = this.getSnapshot();
      next.goals.push(goal);
      next.activities.push(...activities);
      await this.commit(next);
      return { goal: structuredClone(goal), activities: structuredClone(activities) };
    });
  }
  ...
  async updateSessionNote(sessionId: string, note: string): Promise<void> {
    ...
    return this.mutations.run(async () => {
      ...
      session.settlement.userNote = normalizedNote;
      await this.commit(next);
    });
  }
  ...
  async updateSettings(settings: AppSettings): Promise<void> {
    ...
    if (this.historyEnabledIntent && !normalized.ai.historyEnabled) this.historyGeneration += 1;
    this.historyEnabledIntent = normalized.ai.historyEnabled;
    return this.mutations.run(async () => {
      const next = this.getSnapshot();
  ...
  async generateAiGoalDraft(input: string, signal: AbortSignal): Promise<GenerateAiGoalDraftResult> {
    if (!this.aiGoalDraftService) {
      return {
        ok: false,
    ...
    const outcome = await this.aiGoalDraftService.generate({
      settings: this.getSnapshot().settings,
      input,
      signal,
    });
    if (!outcome.interactionCandidate) return { ...outcome, historyWarning: null };

    let historyWarning: AiHistoryWarning = null;
    try {
      await this.mutations.run(async () => {
        const current = this.current();
        if (generation !== this.historyGeneration || !current.settings.ai.historyEnabled) return;
        const appended = appendAiInteraction(current.aiInteractions, outcome.interactionCandidate);
        historyWarning = appended.warning;
        if (!appended.appended) return;
        const next = this.getSnapshot();
        next.aiInteractions = appended.history;
        await this.commit(next);
      });
    } catch {
      historyWarning = 'storage';
    }
    return { ...outcome, historyWarning };
  }
  ...
  clearAiHistory(): Promise<void> {
    ...
    return this.mutations.run(async () => {
      const next = this.getSnapshot();
      if (next.aiInteractions.length === 0) return;
      next.aiInteractions = [];
      await this.commit(next);
    });
  }

  testAiProviderConnection(input: TestAiConnectionInput): Promise<AiConnectionTestOutcome> {
    if (!this.aiGoalDraftService?.testConnection) {
  ...
  async confirmImport(serialized: string): Promise<void> {
    const prepared = this.prepareImport(serialized);
    this.historyGeneration += 1;
    this.historyEnabledIntent = prepared.snapshot.settings.ai.historyEnabled;
    return this.mutations.run(async () => {
  ...
  async clearAllData(): Promise<void> {
    this.historyGeneration += 1;
    this.historyEnabledIntent = false;
    return this.mutations.run(async () => {
